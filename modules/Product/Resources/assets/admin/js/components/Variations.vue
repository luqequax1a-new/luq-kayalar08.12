...
function chooseVariationImage(
    ...
    picker.on("select", ({ id, path }) => {
        ...
        const file = { id: +id, path };
        (form.variants || []).forEach((variant, idx) => {
            const containsValue = (variant.uids || "")
                .split(".")
                .includes(valueUid);

            if (containsValue) {
                if (!Array.isArray(variant.media)) {
                    form.variants[idx].media = [file];
                    return;
                }

                if (variant.media.length === 0) {
                    form.variants[idx].media.push(file);
                } else {
                    form.variants[idx].media.splice(0, 1, file);
                }
            }
        });
    });
