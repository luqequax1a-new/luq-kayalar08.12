...
onMounted(() => {
    ...
    // normalize variant qty numbers (remove trailing zeros like 10.00)
    if (Array.isArray(form.variants)) {
        form.variants.forEach((v) => {
            if (v && v.manage_stock == 1 && v.qty !== undefined && v.qty !== null) {
                const n = Number(v.qty);
                if (!Number.isNaN(n)) v.qty = n;
            }
        });
    }
});
...
const unitStep = computed(() => {
    return selectedUnit.value
        ? Number(
              selectedUnit.value.is_decimal_stock
                  ? selectedUnit.value.step || 0.01
                  : 1,
          )
        : 1;
});
